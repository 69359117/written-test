...
public class t3_1 {
    ...
    public static boolean check(int res) {          // res：窗口的二分
        int[]one = new int[26];                     // 出现次数
        int[]two = new int[26];                     // 是否允许改
        int zNum = 0;                               // zNum：不允许改的元素种类数
        for(int i = 0; i < n; ++i) {
            if(i >= res) {                          // 窗口初始化完毕、滑动窗口
                int last = s.charAt(i - res) - 'a';
                --one[last];
                if(sub.charAt(i - res) == '0') {
                    if(--two[last] == 0) {
                        zNum -= 1;
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        while(T-- != 0) {
            ...
            int resr = n;
            while(resl <= resr) {
                int resmid = (resl + resr) / 2; // resmid：二分法、试探窗口长度
                if(check(resmid)) {
                    resl = resmid + 1;
                }
                else {
